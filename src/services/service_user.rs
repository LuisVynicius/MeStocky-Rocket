...
use crate::{configs::{config_bcrypt::{self, encrypt_password, verify_password}, config_jwt::{self, generate_token, get_email_by_token}}, entities::{dtos::{generic_dtos::ExistsDTO, user_dtos::{AuthenticationDTO, LoginDTO, UserCreateDTO, UserCredentialsUpdateDTO, UserInformationsUpdateDTO, UserSummaryForAdminDTO, UserSummaryForAdminQueryDTO, ValidedTokenDTO}}, enums::user_enums::UserRole, tb_user::{self, ActiveModel, Model}}, errors::BackendError, guards::guard_user::AuthenticationGuard};
...
pub async fn login(
    ...
    match result {
        Ok(user) => {
            ...
            Ok(AuthenticationDTO::new(token.unwrap(), user.role, user.username, UserRole::code_to_string(user.role)))
        },
...
pub async fn valid(
    ...
    let result = exists_by_email(database, &config_jwt::get_email_by_token(&authentication.0)).await;
...
pub async fn update_user_credentials(
    ...
    match result {
        Ok(logged_user) => {
            if !config_bcrypt::verify_password(user_update_dto.get_old_password(), &logged_user.password) {
                return Err(BackendError::InvalidCredentialsError);
            }
